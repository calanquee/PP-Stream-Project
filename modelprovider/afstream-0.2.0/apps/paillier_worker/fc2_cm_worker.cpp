...
int main(int argc, char* argv[]) {
    ...
    afs::Worker<struct ReLUItem, struct VectorItem, afs::NullClass, afs::NullClass>* worker =
        new afs::Worker<struct ReLUItem, struct VectorItem, afs::NullClass, afs::NullClass>(std::string(worker_name), worker_id, std::string(""), num_compute_thread);
    ...
    */
    ...
    if(num_in){
        afs::UpThreadNet<struct ReLUItem, afs::NullClass>* i_thread =
        new afs::UpThreadNet<struct ReLUItem, afs::NullClass>(num_in, num_compute_thread);
    ...
    */
    for (int i=0; i<num_compute_thread; i++) {
        ComputeThreadThread* compute_thread =
            new ComputeThreadThread(num_in, num_downstream);
        worker->AddComputeThread(compute_thread);
    }
