...
*/
typedef struct{
	int bits;  /* e.g., 1024, choose 64 in the experiment*/
	mpz_t n;   /* public modulus n = p q */
	mpz_t n_squared; /* cached to avoid recomputing  n^2*/
	mpz_t n_plusone; /* cached to avoid recomputing g: public key*/
}paillier_pubkey_t;
...
*/
void paillier_add( paillier_pubkey_t* pub,
			paillier_ciphertext_t* res,
			paillier_ciphertext_t* ct0,
			paillier_ciphertext_t* ct1 );

void paillier_sub( paillier_pubkey_t* pub,
			paillier_ciphertext_t* res,
			paillier_ciphertext_t* ct0,
			paillier_ciphertext_t* ct1);
...
void paillier_div(paillier_pubkey_t* pub,
			paillier_ciphertext_t* res,
			paillier_ciphertext_t* ct,
			paillier_plaintext_t* pt);
...
*/
paillier_plaintext_t* paillier_plaintext_from_si(long int x);
...
void formal_paillier_ciphertext_from_str(char *str, int c_length, paillier_ciphertext_t* ct);
