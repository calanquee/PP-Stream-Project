...
AdapterRecovery* OperatorTracker::RecoverOps(RecoveryItem* recovery_item) {
    ...
    for (uint32_t i=0; i<count; i++) {
        ...
        if (bak_item->info.op_index < 65535) {
            if (bak_item->info.worker_id != worker_id_) {
                LOG_ERR("worker id unmatch: %u (recovery) %u (local)\n", bak_item->info.worker_id, worker_id_);
            }
            // LOG_MSG("thread %u, op_index %u, seq %lu\n", bak_item->info.thread_id, bak_item->info.op_index, bak_item->info.seq);
            operators_[bak_item->info.thread_id]->at(bak_item->info.op_index)->RecoveryState(bak_item->data);
            if (bak_item->info.seq > max_seq) {
                max_seq = bak_item->info.seq;
            }
        }
    }
    // data items (see up_thread_afs)
    AdapterRecovery* adapter = new AdapterRecovery();
    adapter->SetMaxSeq(max_seq);
    for (uint32_t i=0; i<count; i++) {
        BackupItem* bak_item = recovery_item->GetSlot(i);
        if (bak_item->info.op_index == 65535) {
            if (bak_item->info.worker_id != worker_id_) {
            ...
            // LOG_MSG("thread %u, op_index %u, seq %lu\n", bak_item->info.thread_id, bak_item->info.op_index, bak_item->info.seq);
            if (bak_item->info.seq > max_seq) {
                adapter->AppendBackupItem(bak_item);
            }
        }
    }
