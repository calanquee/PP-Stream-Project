...
//	printf("init paillier cipher: %d\n",mpz_get_si(paillier_ciphertext.c));
	...
	//printf("paillier_ciphertext.c_length = %d\n", paillier_ciphertext.c_length);
...
int encrypt_image_data(char * filename){
	...
	for(int i=0;i<Dimension;++i){
		for(int j=0;j<Dimension;++j){
			printf("%d ", plain_image_data_invert[i*Dimension+j]);
		}
		printf("\n");
	}
    */
...
int main(int argc, char *argv[]){
    ...
    // image buffer for sending to AF-Stream
    ...
    for(int round=0;round<Test_Round;++round){
		for(int index=0;index<25;++index){
			image_buffer[404544] = index + 1;
			zmq_send(afs_requester, image_buffer, 404545, 0);
		}
		// bzero(image_buffer, 410000);
		// memcpy(image_buffer, &Image, sizeof(Image));
		// usleep(1900000);
