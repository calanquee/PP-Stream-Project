...
std::unordered_map<uint32_t, FILE*> item_backups;
...
public:
    ...
    void AppendRecoveryItem(afs::RecoveryItem* ret, uint32_t cur_count) {
        ...
        for (uint32_t i=0; i<used_slots; i++) {
            ...
            LOG_MSG("recovery seq %lu, worker %d thread %d op_index %lu\n",
                    ret->GetSlot(cur_count+i)->info.seq,
                    ret->GetSlot(cur_count+i)->info.worker_id,
                    ret->GetSlot(cur_count+i)->info.thread_id,
                    ret->GetSlot(cur_count+i)->info.op_index);
        }
        //return ret;
    }
...
std::unordered_map<uint16_t, std::vector<struct BackupContent*>> worker_ops;
...
int main (int argc, char *argv [])
{
    ...
    while (1) {
        ...
        if (backup_item.info.backup_op == 0) { // save backup
            if (backup_item.data.meta.len == 0) { // end
                cur_stop++;
                ZWrite(NULL, 0);
            ...
            else 
            {
                ...
                if (content == NULL) {
                    ...
                    //}
                    worker_ops[backup_item.info.worker_id].push_back(content);
                }
            ...
            for (unsigned int i=0; i<worker_ops[backup_item.info.worker_id].size(); i++) {
                tot_count += worker_ops[backup_item.info.worker_id][i]->GetCount();
            }
            LOG_MSG("tot count %u\n", tot_count);
            ...
            for (unsigned int i=0; i<worker_ops[backup_item.info.worker_id].size(); i++) {
                worker_ops[backup_item.info.worker_id][i]->AppendRecoveryItem(recovery_item, cur_count);
                cur_count += worker_ops[backup_item.info.worker_id][i]->GetCount();
            }
