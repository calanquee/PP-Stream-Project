#ifndef __AFS_IN_CALLBACK_MERGE_HPP_INCLUDED__
#define __AFS_IN_CALLBACK_MERGE_HPP_INCLUDED__
...
template<class T>
class InCallbackMerge : public InCallbackBase {
...
public:
    ...
    InCallbackMerge(int num_in, int num_com) :
        InCallbackBase(num_in, num_com),
        num_stop_(0),
        event_(0),
...
private:
    ...
    int GetDestination(void* data, uint32_t len) {
        ...
        */
        // printf("%d\n", b[24]);
        return b[24]%num_com_;
    }
};

template<class T>
int InCallbackMerge<T>::PushRecord(void* data, uint32_t len) {
    if (afs_unlikely(((WrapT*)data)->get_type()==ITEM_FINISH)) {
        ...
        if (!stopped_[source]) {
            ...
            num_stop_++;
            // LOG_MSG("in callback: forward finish\n");
            for (auto x : queues_) {
                x->Insert(data);
                x->Flush();
            }
            // LOG_MSG("in callback: Num stop %d\n", num_stop_);
    ...
    else if (afs_unlikely(((WrapT*)data)->get_type()==ITEM_EMERGENCE)) {
        ++event_;
        if (event_ == 1) {
            start_ts = now_us();
        }
