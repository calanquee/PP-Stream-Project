 ...
 */
template <class InT>
class UpThreadTraceError : public UpThread<InT, NullClass> {

typedef WrapItem<InT> WInT;

public:
    UpThreadTraceError(RouterBase* router);
...
template<class InT>
UpThreadTraceError<InT>::UpThreadTraceError(
        RouterBase* router
        ) :
...
template<class T>
void UpThreadTraceError<T>::AddOutQueue(ZeroRingBuffer<WInT>* q) {
    queues_.push_back(q);
}

template<class T>
void UpThreadTraceError<T>::ThreadInitHandler() {
    ...
    LOG_MSG("Error freq %lu\n", error_freq_);

    cur_lost_ = 0;
}

template<class T>
void UpThreadTraceError<T>::ThreadMainHandler() {
    LOG_MSG("UpThreadTraceError (%d) start to run\n", getpid());
...
template<class T>
void UpThreadTraceError<T>::ThreadFinishHandler() {
