 ...
 */
template <class InT>
class UpThreadTraceMinibatch : public UpThread<InT, NullClass> {

typedef WrapItem<InT> WInT;

public:
    UpThreadTraceMinibatch(int num_compute_thread, RouterBase* router);
...
template<class InT>
UpThreadTraceMinibatch<InT>::UpThreadTraceMinibatch(
...
template<class T>
void UpThreadTraceMinibatch<T>::AddOutQueue(ZeroRingBuffer<WInT>* q) {
    queues_.push_back(q);
}

template<class T>
void UpThreadTraceMinibatch<T>::ThreadInitHandler() {
...
template<class T>
void UpThreadTraceMinibatch<T>::ThreadMainHandler() {
    LOG_MSG("UpThreadTraceMinibatch (%d) start to run\n", getpid());
...
template<class T>
void UpThreadTraceMinibatch<T>::ThreadFinishHandler() {
