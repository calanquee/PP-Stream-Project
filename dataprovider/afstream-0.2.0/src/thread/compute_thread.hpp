...
protected:
    ...
    */
    ...
    void EmitReverseDataQuick(int dest, ROutT& msg) {
        if (is_stop_[dest] == 1) {
            return;
        }
...
private:
    ...
    void Backups(uint64_t seq) {
        afs_zmq::command_t cmd;
        for (unsigned int i=0; i<num_operator_; i++) {
            ...
            if (d > operators[i]->GetDivergenceThresh()) {
                ...
                if (backup_item.data.meta.len) {
                    // LOG_MSG("Backup at %lu: d %lf thresh %lf\n", seq, d, operators[i]->GetDivergenceThresh());
    ...
    while (up_stop<num_upstream_ || down_stop<num_downstream_) {
        ...
        if (afs_unlikely(up_stop==num_upstream_)) {
            ...
            if ((tuple=in_queue_->Extract()) != NULL) {
                ...
                // if (seq == 0) {
                    // LOG_MSG("Seq %lu type %d event %lu worker %d\n", seq, t, event_, worker);
                //}
                InT data = tuple->data();
                in_queue_->Ack();
                switch (t) {
