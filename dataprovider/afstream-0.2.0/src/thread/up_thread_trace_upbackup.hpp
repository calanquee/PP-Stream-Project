 ...
 */
template <class InT>
class UpThreadTraceUpbackup : public UpThread<InT, NullClass> {

typedef WrapItem<InT> WInT;

public:
    UpThreadTraceUpbackup(int num_compute_thread, RouterBase* router);
...
template<class InT>
UpThreadTraceUpbackup<InT>::UpThreadTraceUpbackup(
    ...
    num_compute_thread_(num_compute_thread) {
        reader_ = new MPSCReader<WNullClass>(num_compute_thread);
    }

template<class T>
void UpThreadTraceUpbackup<T>::AddOutQueue(ZeroRingBuffer<WInT>* q) {
    queues_.push_back(q);
}

template<class T>
void UpThreadTraceUpbackup<T>::ThreadInitHandler() {
    ...
    backup_size = config->getint("sys.fault_tolerance.upbackup_size", 0);
    LOG_MSG("Upbackup size %d\n", backup_size);
    afs_assert(backup_size > 0, "sys.fault_tolerance.upbackup_size should be larger than 0\n");
    backup_data_ = (T*)calloc(sizeof(T), backup_size);
    afs_assert(backup_data_, "allocate backup_data buffer failure\n");

    reader_->Init();
}

template<class T>
void UpThreadTraceUpbackup<T>::ThreadMainHandler() {
    LOG_MSG("UpThreadTraceUpbackup (%d) start to run\n", getpid());
    ...
    while (1) {
        ...
        if (event_ && event_%backup_size==0) {
            EmitTimeout();
            for (int i=0; i<num_compute_thread_; i++) {
                void* data = NULL;
                while (data==NULL) {
        ...
        backup_data_[event_%backup_size] = *((T*)data);
...
template<class T>
void UpThreadTraceUpbackup<T>::ThreadFinishHandler() {
