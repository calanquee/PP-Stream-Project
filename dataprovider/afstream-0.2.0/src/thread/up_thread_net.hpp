...
UpThreadNet<InT, ROutT>::UpThreadNet(
    ...
    callback_choice_(callback_choice),
    //in_callback_(in_callback),
    //out_callbacks_(NULL),
    event_(0) {
        if(callback_choice_==1){
            in_callback_ = new afs::InCallbackMerge<InT>(num_upstream, num_compute_thread);
        }
        else{
            in_callback_ = new afs::InCallbackSimple<InT>(num_upstream, num_compute_thread);
        }
