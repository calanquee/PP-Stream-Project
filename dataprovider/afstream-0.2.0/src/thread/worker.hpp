...
void Worker<InT, OutT, RInT, ROutT>::Start(int recovery) {
    ...
    if (recovery > 0) {
        ...
        AdapterRecovery* adapter_recovery = OperatorTracker::GetInstance()->RecoverOps(recovery_item);
        backup_client->EndRecovery(recovery_item);
        up_thread_->SetAdapterRecovery(adapter_recovery);
    ...
    while (1) {
        ...
        if (rc == -1) {
            ...
            // data backup
            if (backup_client) {
                //LOG_MSG("Backup data %lu thread %d worker %d, seq %lu\n",
                //        cmd.args.backup_item.info.seq, cmd.args.backup_item.info.thread_id, cmd.args.backup_item.info.worker_id, cmd.args.backup_item.info.seq);
                backup_client->MakeBackup(cmd.args.backup_item);
            }
            up_thread_->NotifyThread(afs_zmq::command_t::type_t::backup);
        }
        else {
