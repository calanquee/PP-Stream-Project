...
afs_zmq::tcp_connecter_t::tcp_connecter_t (
...
void afs_zmq::tcp_connecter_t::process_plug ()
{
    if (delayed_start)
        add_reconnect_timer ();
    else
...
// qhuang
void afs_zmq::tcp_connecter_t::set_poller (epoll_t* poller)
{
...
void afs_zmq::tcp_connecter_t::terminate ()
{
    if (timer_started) {
...
bool afs_zmq::tcp_connecter_t::in_event ()
{
...
bool afs_zmq::tcp_connecter_t::out_event ()
{
    fd_t fd = connect ();
    rm_fd (handle);
    handle_valid = false;
...
void afs_zmq::tcp_connecter_t::timer_event (int id_)
{
...
void afs_zmq::tcp_connecter_t::start_connecting ()
{
...
void afs_zmq::tcp_connecter_t::add_reconnect_timer()
{
...
int afs_zmq::tcp_connecter_t::get_new_reconnect_ivl ()
{
...
int afs_zmq::tcp_connecter_t::open ()
{
    zmq_assert (s == retired_fd);
...
afs_zmq::fd_t afs_zmq::tcp_connecter_t::connect ()
{
...
void afs_zmq::tcp_connecter_t::close ()
{
    zmq_assert (s != retired_fd);
    int rc = ::close (s);
    errno_assert (rc == 0);
